...
function runSummaryHtml(run){
  const agents=run.subtasks.filter(s=>!s.human),done=agents.filter(s=>['completed','error','cancelled','stopped'].includes(s.status)).length;
  const label=run.status==='awaiting-plan'?'Plan a la espera de tu aprobación':(run.stage||stageNames[run.status]||run.status);
  return `<div class="plan-head"><strong>${esc(label)}</strong><span>${agents.length?`<span class="pill">${done}/${agents.length}</span>`:''}${run.usage?.total?` <span class="pill">${usageText(run.usage)}</span>`:''}</span></div>`;
}
function renderRun(){
  ...
  const key=signature+runCardCollapsed;
  if(key===lastRun)return;lastRun=key;
  if(!run){status.hidden=true;status.innerHTML='';status.classList.remove('collapsed');return;}
  let html='';
  if(run.status==='awaiting-plan')html=planHtml(run);
  else if(ACTIVE.includes(run.status))html=teamHtml(run);
  else html=afterRunHtml(run); // el error de una tarea terminada se lee dentro de la conversación
  if(!html.trim()){status.hidden=true;status.innerHTML='';return;}
  status.hidden=false;status.classList.toggle('collapsed',runCardCollapsed);
  status.innerHTML=(runCardCollapsed?runSummaryHtml(run):html)+`<button type="button" id="run-toggle" class="card-toggle" title="${runCardCollapsed?'Desplegar la tarjeta de la tarea':'Plegar la tarjeta a una línea'}">${runCardCollapsed?'▸':'▾'}</button>`;
  $('#run-toggle').onclick=()=>{runCardCollapsed=!runCardCollapsed;lastRun='';renderRun();if(following)scrollMessagesToBottom();};
}
...
function updateOrchestrator(){
  ...
  $('#quota-hint').classList.toggle('warn',codexUse>=80);
  if(codexUse>=80&&orchestrator==='codex'&&!quotaWarned){quotaWarned=true;toast(`Codex lleva el ${Math.round(codexUse)} % de su cuota. Cambia a Claude Code como agente principal para no quedarte sin ella.`);}
...
async function renderRepoPicker(refresh=false){
  ...
  box.innerHTML='<p class="muted">Leyendo tus repositorios…</p>';
  let repos;try{repos=(await api('github/repos'+(refresh?'?refresh=1':''))).repos;}catch(error){box.innerHTML=`<div class="message-error">${esc(error.message)}</div>`;return;}
  const draw=()=>{
    const q=($('#repo-search')?.value||'').trim().toLowerCase();
    const shown=repos.filter(r=>!q||r.fullName.toLowerCase().includes(q)||(r.description||'').toLowerCase().includes(q)).slice(0,60);
    box.innerHTML=shown.length?`<div class="repo-list">${shown.map(r=>`<div class="repo-row"><div><strong>${esc(r.fullName)}</strong>${r.private?' <span class="pill">privado</span>':''}${r.language?` <span class="muted">${esc(r.language)}</span>`:''}${r.description?`<div class="muted">${esc(r.description.slice(0,140))}</div>`:''}</div>${r.project?`<button type="button" class="secondary-button" data-open-project="${esc(r.project)}">Abrir</button>`:`<button type="button" class="primary-button" data-clone="${esc(r.fullName)}">Clonar</button>`}</div>`).join('')}</div>`:'<p class="muted">Ningún repositorio coincide.</p>';
  };
  draw();
