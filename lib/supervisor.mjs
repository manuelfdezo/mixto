...
import {isIgnored} from './isolation.mjs';
  ...
  async poll(){
    ...
    for(const subtask of this.subtasks){
      ...
      try{
        ...
        // El ruido propio del agente (p.ej. `.atl`) nunca debe generar colisión ni invasión.
        snapshots.push({index:subtask.index,scope:subtask.scope||[],files:statusFiles(stdout).filter(file=>!isIgnored(file))});
