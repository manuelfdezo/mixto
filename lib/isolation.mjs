...
export async function initRepository(projectPath) {
  ...
  await run('git',['-C',projectPath,...IDENTITY,'commit','--no-verify','-m','mixto: punto de partida'],{windowsHide:true,maxBuffer:MAX_BUFFER});
}
...
export async function createWorkspaces({projectPath,dataDir,runId,indexes,sharedPaths=DEFAULT_SHARED}) {
  ...
  const warnings=[];
  let base=info.head;
  if(info.dirtyTracked.length) {
    // `stash create` records the dirty tracked state as a commit without touching the working tree.
    const stashed=(await git(info.toplevel,['stash','create'])).trim();
    if(stashed)base=stashed;
    else warnings.push('No se pudo incluir el trabajo sin guardar en las copias aisladas; los agentes parten del último commit.');
  }
  ...
  for(const index of indexes) {
    ...
    // Everything present before the agent starts becomes the baseline commit, so its patch is only its own work.
...
export async function capturePatch({worktreeRoot,patchPath}) {
  await git(worktreeRoot,['add','-A','.']);
  const numstat=(await git(worktreeRoot,['diff','--numstat','HEAD'])).trim();
  if(!numstat)return null;
  const diff=await git(worktreeRoot,['diff','--binary','HEAD']);
  ...
  return {file:patchPath,files,insertions,deletions};
}
