...
const excludeArgs=paths=>(paths||[]).filter(Boolean).map(entry=>`--exclude=${entry}`);
const includeArgs=paths=>(paths||[]).filter(Boolean).map(entry=>`--include=${entry}`);

export async function patchFiles({projectPath,patchFile,excludes=[]}) {
  const output=await git(projectPath,['apply','--numstat',...excludeArgs(excludes),patchFile]);
  return lines(output).map(line=>line.split('\t')[2]).filter(Boolean);
}

// Divide un diff unificado por archivo, con estado y recuento, para mostrarlo y actuar archivo a archivo.
export function parseDiff(text) {
  const files=[];let current=null;
  for(const line of String(text||'').split('\n')) {
    const header=/^diff --git "?a\/(.+?)"? "?b\/(.+?)"?$/.exec(line);
    if(header) {current={path:header[2],oldPath:header[1],status:'modified',binary:false,additions:0,deletions:0,lines:[line]};files.push(current);continue;}
    if(!current)continue;
    current.lines.push(line);
    if(/^new file mode/.test(line))current.status='added';
    else if(/^deleted file mode/.test(line))current.status='deleted';
    else if(/^rename from /.test(line))current.status='renamed';
  ...
  return files.map(file=>({path:file.path,oldPath:file.oldPath,status:file.status,binary:file.binary,additions:file.additions,deletions:file.deletions,text:file.lines.join('\n')}));
}

// Deshace un parche ya aplicado, entero o solo algunos archivos; comprueba antes de tocar nada.
export async function revertPatch({projectPath,patchFile,includes=[]}) {
  let text='';
  try {text=fs.readFileSync(patchFile,'utf8');}
  catch {return {ok:false,error:'El diff ya no está disponible.',reverted:[]};}
  // Un turno que solo creó archivos deja un diff sin parches: no hay nada que pedirle a git.
  const wanted=parseDiff(text).filter(file=>!includes.length||includes.includes(file.path)).map(file=>file.path);
  if(!wanted.length)return {ok:true,error:null,reverted:[]};
  const args=['apply','-R',...includeArgs(includes)];
  try {await git(projectPath,[...args,'--check',patchFile]);}
  catch(error) {return {ok:false,error:reasonOf(error),reverted:[]};}
  try {await git(projectPath,[...args,patchFile]);return {ok:true,error:null,reverted:wanted};}
  catch(error) {return {ok:false,error:reasonOf(error),reverted:[]};}
}

function statusEntries(output) {
  return lines(output).map(line=>{
    const code=line.slice(0,2),entry=line.slice(2).trim();
    const renamed=entry.split(' -> ');
    const file=(renamed[renamed.length-1]||'').replace(/^"|"$/g,'');
    const status=code==='??'?'untracked':/D/.test(code)?'deleted':/A/.test(code)||code.includes('?')?'added':/R/.test(code)?'renamed':'modified';
    return {path:file,status,code};
  }).filter(entry=>entry.path);
}

// Lo que hay sin confirmar en el proyecto: archivos, diff de los que tienen seguimiento y nuevos aparte.
export async function projectChanges(projectPath,{maxDiff=400000,withDiff=true}={}) {
  const info=inspect(projectPath);
  if(info.kind!=='worktree')return {git:false,toplevel:null,files:[],untracked:[],diff:'',status:''};
  const status=await git(info.toplevel,['status','--porcelain','--untracked-files=all']);
  const files=statusEntries(status).filter(entry=>!isIgnored(entry.path));
  const untracked=files.filter(entry=>entry.status==='untracked').map(entry=>entry.path);
  const exclude=ignoredPaths().map(entry=>`:(exclude)${entry}`);
  let diff='';
  if(withDiff) {try {diff=await git(info.toplevel,['diff','HEAD','--',...exclude]);} catch {try {diff=await git(info.toplevel,['diff','--',...exclude]);} catch {}}}
  return {git:true,toplevel:info.toplevel,files,untracked,diff:diff.slice(0,maxDiff),truncated:diff.length>maxDiff,status:status.trim()};
}

// Confirma exactamente los archivos elegidos, con la identidad de git del usuario; nunca empuja.
export async function commitFiles({projectPath,message,files}) {
  const info=inspect(projectPath);
  if(info.kind!=='worktree')throw new Error('La carpeta no es un repositorio git.');
  const email=(await git(info.toplevel,['config','--get','user.email']).catch(()=>'')).trim();
  if(!email)throw new Error('Tu git no tiene user.name y user.email configurados; configúralos para confirmar con tu identidad.');
  const chosen=(files||[]).filter(entry=>typeof entry==='string'&&entry.trim());
  if(!chosen.length)throw new Error('Elige al menos un archivo que confirmar.');
  await git(info.toplevel,['add','-A','--',...chosen]);
  await git(info.toplevel,['commit','-q','-m',message,'--',...chosen]);
  return {hash:(await git(info.toplevel,['rev-parse','--short','HEAD'])).trim()};
}

export async function pushBranch(projectPath) {
  const info=inspect(projectPath);
  if(info.kind!=='worktree')return {ok:false,error:'La carpeta no es un repositorio git.'};
  try {
    const out=await run('git',['-C',info.toplevel,'push','--quiet'],{windowsHide:true,maxBuffer:MAX_BUFFER,env:{...process.env,GIT_TERMINAL_PROMPT:'0'},timeout:60000});
...
export async function checkPatches({projectPath,patches,excludes={}}) {
  ...
  for(const patch of list) {
    const skip=excludes[patch.file]||[];
    let touched=[];
    try {touched=await patchFiles({projectPath,patchFile:patch.file,excludes:skip});}
    ...
    try {await git(projectPath,['apply','--check',...excludeArgs(skip),patch.file]);}
...
// `excludes` deja fuera archivos concretos de un parche (el usuario los descartó en el visor de cambios).
export async function applyPatches({projectPath,patches,excludes={}}) {
  const list=(patches||[]).filter(patch=>patch?.file);
  if(!list.length)return {applied:[],conflicts:[]};
  const verified=await checkPatches({projectPath,patches,excludes});
  if(!verified.ok)return {applied:[],conflicts:verified.failures};
  const applied=[];
  for(const patch of list) {
    const skip=excludes[patch.file]||[];
    try {
      const touched=await patchFiles({projectPath,patchFile:patch.file,excludes:skip});
      if(touched.length)await git(projectPath,['apply',...excludeArgs(skip),patch.file]);
      applied.push(patch.file);
    }
    catch(error) {
      // All or nothing: undo what already landed so the project never stays half-integrated.
      for(const done of [...applied].reverse()) {try {await git(projectPath,['apply','-R',...excludeArgs(excludes[done]||[]),done]);} catch {}}
