...
export class Store {
  constructor(directory, workspace) {
    ...
    this.data.settings={codexInstructions:'',claudeInstructions:'',orchestratorPersona:'',autoApproveSingle:false,autoApproveReadOnly:false,tokenBudget:0,balance:'auto',reviewPolicy:'multi',modelNotes:'',claudeSoftLimit:0,autoPull:true,...(this.data.settings||{})};
    // Personas reales del equipo: una lista global y, por proyecto, quiénes forman parte de él.
    if(!Array.isArray(this.data.people))this.data.people=[];
    if(!Array.isArray(this.data.uploads))this.data.uploads=[];
    for(const project of this.data.projects){
      if(!Array.isArray(project.members))project.members=[];
      if(!Array.isArray(project.assignments))project.assignments=[];
      project.cooperation={enabled:false,autoPublish:true,lastSync:null,lastPublish:null,pendingPublish:false,error:null,identity:null,remote:null,...(project.cooperation||{})};
    }
