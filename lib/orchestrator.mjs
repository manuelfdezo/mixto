...
export function buildPlanPrompt({request,connections,memory='',history='',instructions='',maxSubtasks=8,maxParallel=3,readOnly=true}) {
...
function normalizeSubtask(item,position,{connections,runReadOnly,warnings}) {
...
export function parseManualPlan(plan,{connections,maxSubtasks=8,runReadOnly=false}={}) {
  ...
  const subtasks=proposed.map((item,index)=>{
    ...
    return normalizeSubtask({
      titulo:item?.title,proveedor:item?.provider,modelo:item?.model,esfuerzo:item?.effort||null,
    ...
    },index+1,{connections,runReadOnly,warnings});
  });
...
export function parsePlan(text,{connections,maxSubtasks=8,runReadOnly=false}={}) {
  ...
  const subtasks=kept.map((item,index)=>normalizeSubtask(item,index+1,{connections,runReadOnly,warnings}));
...
export function buildWorkPrompt({request,subtask,memory='',instructions='',context='',direct=false}) {
  return [
