...
export function buildPlanPrompt({request,connections,memory='',history='',instructions='',maxSubtasks=8,maxParallel=3,readOnly=true,people=[]}) {
  const team=(people||[]).map(person=>`  - ${person.name}${person.role?` · ${person.role}`:''}${person.notes?` · ${String(person.notes).slice(0,200)}`:''}`).join('\n');
