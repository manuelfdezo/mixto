...
export function buildPlanPrompt({request,connections,memory='',history='',instructions='',maxSubtasks=8,maxParallel=3,readOnly=true,people=[],balance='auto',routing='',repo=''}) {
  ...
  return [
    ...
    ['CÓMO ASIGNAR CADA SUB-TAREA (pondera las cuatro cosas, en este orden):',
      '1. Cuota restante: un agente con poca cuota (30 % o menos) solo recibe trabajo si es imprescindible; sin cuota, nunca. Prefiere el agente al que más cuota le queda cuando los dos sirven.',
      '2. Magnitud: estima si la sub-tarea es pequeña (un archivo, un cambio concreto), mediana (una función o un flujo con sus tests) o grande (varios archivos, diseño o depuración difícil), y anótala en el campo "magnitud". Pequeña → modelo rápido y nivel bajo; mediana → modelo equilibrado; grande → modelo potente y nivel alto.',
      '3. Fortalezas: usa las notas sobre modelos de arriba para decidir a qué agente y modelo se le da mejor ese tipo de trabajo.',
      '4. Preferencia del usuario, si la hay.',
      'En "justificacion" resume en una frase por qué ese agente y modelo (magnitud, cuota, fortaleza).'].join('\n'),
    ...
    ['Responde con UN ÚNICO bloque ```json y nada fuera del bloque, con una de estas dos formas exactas.',
      ...
      ' "subtareas":[{"titulo":"","proveedor":"codex|claude|persona","persona":"<nombre exacto, solo si proveedor es persona>","modelo":"<id exacto del catálogo, o null para una persona>","esfuerzo":"<nivel o null>","magnitud":"pequeña|mediana|grande",',
...
const magnitudeOf=value=>['pequeña','mediana','grande'].includes(String(value||'').trim().toLowerCase())?String(value).trim().toLowerCase():null;
function normalizeSubtask(item,position,{connections,runReadOnly,warnings,people=[]}) {
  ...
  const common=()=>({
    ...
    magnitude:magnitudeOf(item?.magnitud),
  ...
  if(provider==='auto'){
    if(!PROVIDERS.some(candidate=>connections?.[candidate]?.connected))throw new PlanError(`Sub-tarea ${position}: ningún agente tiene sesión iniciada.`);
    return {...common(),provider:'auto',auto:true,model:'',effort:null,readOnly:runReadOnly||item?.soloLectura===true,magnitude:magnitudeOf(item?.magnitud)};
  }
...
export function buildWorkPrompt({request,subtask,memory='',instructions='',context='',direct=false,teamNote='',repo=''}) {
  return [
...
export function buildDirectPrompt({request,memory='',instructions='',history='',readOnly=true,resumed=false,repo=''}) {
  return [
