...
export const executables={codex:process.env.MIXTO_CODEX_PATH||'codex',claude:process.env.MIXTO_CLAUDE_PATH||'claude'};
...
class Lines extends EventEmitter {
  constructor(provider,args,cwd) {
    ...
    const command=executables[provider];
    this.child=spawn(Array.isArray(command)?command[0]:command,[...(Array.isArray(command)?command.slice(1):[]),...args],{cwd,windowsHide:true,stdio:['pipe','pipe','pipe']});
...
export async function discover(provider,cwd){
  ...
  try{
    ...
    const {stdout}=await execFileAsync(executables.claude,['auth','status'],{windowsHide:true,timeout:15000,maxBuffer:1024*1024}).catch(error=>{if(error.stdout)return {stdout:error.stdout};throw error;});
