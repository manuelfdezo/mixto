...
async function codexConnection(cwd){
  ...
  try{await io.request('initialize',{clientInfo:{name:'mixto',title:'Mixto',version:'1.1.0'}});io.send({method:'initialized',params:{}});return io;}
...
async function runClaude(o){
  ...
  if(o.readOnly){
    args.push('--tools',['Read','Glob','Grep',...(o.extraTools||[])].join(','),'--safe-mode');
