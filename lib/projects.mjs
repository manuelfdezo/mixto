...
export function syncDiscoveredProjects(data,root,stamp=()=>new Date().toISOString()) {
  ...
  for(const item of discovered.projects) {
    ...
    const project={id:randomUUID(),name:item.directoryName,path:item.path,description:'',createdAt:stamp(),managed:true,directoryName:item.directoryName,members:[]};
...
export function createManagedProject(data,root,{name,directoryName,description=''}) {
  ...
  const project={id:randomUUID(),name:name.trim(),path:projectPath,description:description.trim(),createdAt:new Date().toISOString(),managed:true,directoryName,members:[]};
